"""Tests for the Cycle 26 Fix 1 post-S2 ghost-fact validator."""
...
def test_rejects_user_is_other_persons_spouse():
    ...
    keep, reason = _validate_s2_fact(fact, "Mary Chen", ["Mary", "I", "me"])
    assert keep is False
    assert reason == "identity"


def test_rejects_mary_is_danas_husband():
    fact = _make_fact("Mary Chen is Dana's husband", entities=["Dana"], category="relationship")
    keep, _ = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is False


def test_allows_mary_is_danas_colleague():
    fact = _make_fact("Mary Chen is Dana's colleague", entities=["Dana"], category="relationship")
    keep, _ = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is True
...
def test_rejects_mary_is_twin_brother_of_someone():
    fact = _make_fact("Mary Chen is the twin brother of Priya", entities=["Priya"], category="relationship")
    keep, _ = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is False


def test_rejects_mary_is_cousin_named_dana():
    fact = _make_fact("Mary Chen is a cousin named Dana", entities=["Dana"], category="relationship")
    keep, _ = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is False
...
def test_rejects_twin_brother_named_mary():
    fact = _make_fact(
        "User has a twin brother named Mary", entities=["Mary"],
        category="relationship",
    )
    keep, reason = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is False
...
def test_rule3a_does_not_trigger_on_verb_called():
    """'we called Mary to discuss' is a phone-call verb, not a naming.
    Rule 3a must not drop legitimate facts on this phrasing."""
    fact = _make_fact(
        "User called Mary to discuss the project", entities=["Mary"],
        category="relationship",
    )
    keep, _ = _validate_s2_fact(fact, "Mary Chen", ["Mary"])
    assert keep is True
...
def test_writer_process_filters_ghost_fact_end_to_end(monkeypatch, tmp_path):
    ...
    w = MemoryWriter(
        ...
        num_ctx=4096,
        owner_name="Mary Chen",
        profile_owner_aliases=["Mary", "I", "me"],
...
# ── Rule 4 (Cycle 28 Option B1): relative-cohabitation ──────────────────────

def test_rule4_rejects_lives_with_known_child():
    fact = _make_fact(
        "Mary Chen lives with a neighbor named Jake",
        entities=["Jake"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives={"jake", "ethan"},
    )
    assert keep is False
...
def test_rule4_rejects_lives_with_spouse():
    fact = _make_fact(
        "Mary Chen lives with Tom",
        entities=["Tom"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives={"tom"},
    )
    assert keep is False
    assert reason == "relative_cohabitation"
...
def test_rule4_rejects_resides_with_sibling():
    fact = _make_fact(
        "Mary resides with Dana",
        entities=["Dana"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives={"dana"},
    )
    assert keep is False
    assert reason == "relative_cohabitation"
...
def test_rule4_allows_lives_with_nonrelative():
    fact = _make_fact(
        "Mary Chen lives with Amanda",
        entities=["Amanda"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives={"jake", "ethan", "tom"},
    )
    assert keep is True
...
def test_rule4_no_op_when_relatives_empty():
    ...
    fact = _make_fact(
        "Mary Chen lives with Jake",
        entities=["Jake"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives=set(),
    )
    assert keep is True
    assert reason is None
...
def test_rule4_allows_lives_in_place():
    """'Mary Chen lives in Boston' should not match 'lives with X'."""
    fact = _make_fact(
        "Mary Chen lives in Boston",
        category="residence",
    )
    keep, _ = _validate_s2_fact(
        fact, "Mary Chen", ["Mary"], relatives={"jake", "tom"},
    )
    assert keep is True


def test_rule4_handles_user_alias():
    """Rule 4 should fire on 'User lives with Jake' too, not just 'Mary'."""
    fact = _make_fact(
        "User lives with Jake",
        entities=["Jake"], category="residence",
    )
    keep, reason = _validate_s2_fact(
        fact, "Mary Chen", ["Mary", "I", "me"], relatives={"jake"},
    )
    assert keep is False
    assert reason == "relative_cohabitation"
